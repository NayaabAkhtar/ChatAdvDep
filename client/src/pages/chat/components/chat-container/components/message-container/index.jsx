...
const MessageContainer = () => {
  ...
  const renderMessage = () => {
    ...
    return selectedChatMessages.map((message, index) => {
      ...
      return (
        <div key={index}>
          {showDate && (
            <div className="my2 text-center text-gray-500">
    ...
    >
      {message.messageType === "text" && (
        <div
          ...
          } border inline-block p-4 rounded-2xl my-1 break-words max-w-[70%]`}
        >
          {message.content}
        </div>
      )}
        ...
        <div
          className={`${
            message.sender !== selectedChatData._id
              ? "bg-[#8417ff]/5 text-white border-[#8417ff]/50"
              : "bg-[#2a2b33]/5 text-white border-[#ffffff]/20"
          } border inline-block p-4 rounded-2xl my-1 break-words max-w-[85%] sm:max-w-[70%]`}
        >
          ...
          </div> ): (<div className="flex justify-center items-center sm:gap-4">
        ...
        <div
          className={`${
            message.sender._id === userInfo.id
              ? "bg-[#8417ff]/5 text-white border-[#8417ff]/50"
              : "bg-[#2a2b33]/5 text-white border-[#ffffff]/20"
          } border inline-block p-4 rounded-2xl my-1 break-words sm:max-w-[85%] max-w-[70%]`}
        >
          {checkIfImage(message.fileUrl) ? (<div className="cursor-pointer" 
          onClick={()=>{
            setShowImage(true)
      ...
      { message.sender._id!==userInfo.id ? (<div className="flex items-center justify-start gap-3">
          <Avatar className="w-8 h-8 rounded-full overflow-hidden">
             {message.sender.image && (
               ...
               <AvatarFallback
                 className={`uppercase w-8 h-8 text-lg flex items-center justify-center rounded-full ${getColor(message.sender.color)}`}>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 px-8 scrollbar-hidden w-full md:w-[65vw] lg:w-[70vw] xl:w-[80vw]">
      {renderMessage()}
      <div ref={scrollRef} />
      {
